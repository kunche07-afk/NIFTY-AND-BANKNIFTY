...
def leg_cols(leg, prefix):
    ...
    if leg is None:
        values = {f"{prefix} LTP": None, f"{prefix} ATH": None, f"{prefix} Drop %": "\u2014"}
        triggers = {}
        for key, _ in keys:
            values[f"{prefix} {key}"] = "\u2014"
    ...
    values = {
        ...
        f"{prefix} ATH": round(ath, 2),
        f"{prefix} Drop %": f"{d:.2f}%",
    }
    triggers = {}
...
col_order = [
    "CE LTP", "CE ATH", "CE Drop %", "CE \u22657%", "CE \u22655%", "CE \u22653%",
    "Strike",
    "PE \u22653%", "PE \u22655%", "PE \u22657%", "PE Drop %", "PE ATH", "PE LTP",
]
