...
def pct_str(d, threshold):
    return f"{d:.2f}%" if d is not None and d >= threshold else "\u2014"
...
def leg_cols(leg, prefix):
    if leg is None:
        return {
            ...
            f"{prefix} \u22657%": "\u2014", f"{prefix} \u22655%": "\u2014", f"{prefix} \u22653%": "\u2014",
        }
    d = leg["drop_pct"]
    return {
        f"{prefix} LTP": round(leg["ltp"], 2),
        f"{prefix} ATH": round(leg["ath"], 2),
        f"{prefix} \u22657%": pct_str(d, config.THRESHOLDS["severe"]),
        f"{prefix} \u22655%": pct_str(d, config.THRESHOLDS["moderate"]),
        f"{prefix} \u22653%": pct_str(d, config.THRESHOLDS["mild"]),
    }
